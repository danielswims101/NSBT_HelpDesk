...
import { DENIED_MESSAGE, isAllowedAdminEmail } from "../admin-allowlist";
...
const NSBT_DESK_ORIGINS = ["https://desk.nsbt.org", "https://www.desk.nsbt.org"];

// Origins Better Auth accepts on credentialed POSTs (sign-up/sign-in, etc.).
// Missing entries here surface as FORBIDDEN "Invalid origin".
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...LOCAL_DEV_ORIGINS, ...NSBT_DESK_ORIGINS]
  : [
...
export const auth = betterAuth({
  ...
  // Local email/password is off. Google Workspace only, and only the named NSBT admins.
      ...
      create: {
        before: async (user) => {
          if (!isAllowedAdminEmail(user.email)) {
            throw new APIError("FORBIDDEN", { message: DENIED_MESSAGE });
          }
          return { data: user };
        },
      },
